...
| T8 | R42 | Reduce | Wrap `importDriveLinks`, `extractText`, `prepRecipe`, and `sampleRowsToEvaluation` in try/catch and return scrubbed error text instead of letting raw Drive/Sheets exceptions reach the client via the default `google.script.run` failure serialization. Implemented 2026-08-17 (AI-79): each of the four, plus `runBatchAI` as a defense-in-depth backstop (its existing per-row error handling already covered most failure modes, but not a catastrophic exception before any row is processed), now wraps its full body in an explicit try/catch — deliberately not a shared wrapper/HOF over the RPC boundary, since that would be a bigger structural precedent than the problem (a five-function gap caught in one pass, not a recurring pattern like T6/R45) warrants. Each catch calls `logUnexpected` then throws `new Error(toSafeMessage(e, "<Tool> failed — see script logs"))`, which the client's existing `alert("Error: " + err.message)` renders unchanged. `extractText`'s "column not found" error was promoted to a `DomainError` so it survives the wrapping catch verbatim — it names a column the user themselves selected in the sidebar, not internal detail |
