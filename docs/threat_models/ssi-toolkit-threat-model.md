...
| T12 | Stackdriver logging captures sensitive data | A2, A3, C2 | `appsscript.json` routes exceptions to Stackdriver. If error handling includes cell values or file names in exception messages, sensitive journalist data lands in logs accessible to anyone with GCP project access. Audited 2026-08-17 (AI-79): prior to this fix there was no logging in the codebase at all (`Logger.log`/`console.*` were unused everywhere), so the risk was latent rather than realized. `src/server/error-handling.ts`'s `logError()` is now the sole logging entry point across the server and never logs a caught exception's `.message` — only its type (`e.constructor.name`) plus caller-supplied `meta` restricted to `number \| boolean`, which cannot carry cell content, file names, or other free text |
...
| T12 | R14 | Reduce | Audit exception handling in `index.ts` to ensure cell values, file names, and API responses are not included in thrown error messages; log structured codes rather than raw data. Implemented 2026-08-17 (AI-79) — see R25/R42 and `src/server/error-handling.ts` |
...
| T17 | R25 | Reduce | Return generic error strings from `runInference()`'s and `SSI()`'s catch blocks instead of raw exception messages; log the full exception server-side only. Implemented 2026-08-17 (AI-79): `SSI()`'s catch now uses `error-handling.ts`'s `DomainError`/`toSafeMessage`/`logUnexpected` — a `DomainError` (Gemini's own structured API error, an unknown-tool validation message, the missing-API-key message) passes through verbatim since it was deliberately authored to be user-safe; anything else becomes `[Error: ${GENERIC_FAILURE_MESSAGE}]` and is logged (type only, never the message). `GENERIC_FAILURE_MESSAGE` deliberately avoids "see script logs" — most users have sheet-edit access but not Apps Script project access, so they can't see Cloud Logging at all; the copy is "Something went wrong — try again, or contact an admin if it persists." instead. `runInference()` was found to have no production caller during this fix (`buildInferenceRequest` + `callGeminiAPIBatch` replaced it in `runBatchAI`) and was left unchanged — parked in AI-104 (under AI-55) for removal rather than fixed, since scrubbing dead code's error messages has no user-facing benefit |
...
| T8 | R42 | Reduce | Wrap `importDriveLinks`, `extractText`, `prepRecipe`, and `sampleRowsToEvaluation` in try/catch and return scrubbed error text instead of letting raw Drive/Sheets exceptions reach the client via the default `google.script.run` failure serialization. Implemented 2026-08-17 (AI-79): each of the four, plus `runBatchAI` as a defense-in-depth backstop (its existing per-row error handling already covered most failure modes, but not a catastrophic exception before any row is processed), now runs its body through `error-handling.ts`'s `withErrorScrubbing(site, fn)` — a plain function, not an export-wrapping HOF (the export shape and Rollup footer contract are untouched), extracted after review found the initial five hand-copied try/catch blocks were themselves an unmaintained-duplication risk, and that `index.ts` being excluded from coverage meant that logic had zero test coverage; `withErrorScrubbing` lives in `error-handling.ts`, which has none of the GAS-global coupling that excludes `index.ts`, so it's fully unit tested. Each call logs (type only) then throws `new Error(toSafeMessage(e, GENERIC_FAILURE_MESSAGE))`, rendered by the client's `alert(err.message)`. `extractText`'s "column not found" error was promoted to a `DomainError` so it survives the wrapping catch verbatim — it names a column the user themselves selected in the sidebar, not internal detail. Reconsidered whether RPC-level scrubbing was warranted at all, given the catastrophic case (API key in an exception message) is already closed independently by T17/R24 and an RPC failure is only ever shown to the same user who already has access to the resource in question (unlike a cell-write error, which can persist for a different, lower-privileged future viewer) — kept the scrubbing (matches the existing T12/R14 policy and insures against exception content from layers this code doesn't fully control) but fixed the fallback copy, which previously told users to "see script logs" despite most of them having no access to Cloud Logging. Manual QA surfaced two follow-up fixes: (1) every client panel's `.catch` was displaying `"Error: " + err.message`; since `google.script.run` relays a thrown Error's `toString()` as `err.message` — always `"Error: <message>"`, as neither the built-in `Error` nor `DomainError` override `.name` — this rendered a doubled `"Error: Error: ..."`. An initial fix normalized this in `services.ts` by stripping the redundant prefix from every `err.message` before it reached a panel, but the actual duplicate token was the client's own `"Error: "` literal, not something GAS's serialization needed correcting — removed that literal from every panel instead (`alert(err.message)`, or `"Couldn't load headers: " + err.message` for panels with contextual framing), which fixes the same bug without depending on an assumption about exactly what GAS's cross-boundary serialization does. (2) `importDriveLinks` and `prepRecipe`'s `list-drive-folder` fill strategy both called `DriveApp.getFolderById` directly with no handling, so a malformed URL or an inaccessible/nonexistent folder fell through to the generic `GENERIC_FAILURE_MESSAGE` instead of something actionable. An initial fix added a single-purpose `getDriveFolderOrThrow()` wrapper in `drive.ts`, but a wrapper function per risky operation doesn't scale as more call sites need this same treatment. Replaced with `withActionableError(fn, message)` in `error-handling.ts` — a general, reusable primitive that converts anything `fn()` throws into a `DomainError` with the given message; call sites use it inline (`withActionableError(() => DriveApp.getFolderById(folderId), FOLDER_NOT_FOUND_MESSAGE)`) with no new named function required, here or at any future site with the same need. `FOLDER_NOT_FOUND_MESSAGE` — "Could not find that Drive folder — check the link and that you have access to it" — is shared between the two call sites in `index.ts`; GAS doesn't reliably distinguish a malformed ID from a valid-but-inaccessible one, so the message doesn't claim more precision than that |
...
| High | Closed | [AI-79](https://linear.app/propublica/issue/AI-79/fix-t17-raw-exception-messages-echoed-into-spreadsheet-cells) | — | Fix T17 — echoed exceptions | Done 2026-08-17 — `SSI()`'s catch scrubs raw exception messages via `error-handling.ts` (R25). Scope widened during the fix to also close R42 (see below) and audit T12. `runInference()` left unchanged (dead code) — parked in AI-104 |
