...
**Independence of template copies (AI-112):** `File → Make a copy` on the template Sheet (C13) produces a wholly independent Apps Script project — its own script ID, its own Script Properties, its own OAuth consent. No `GEMINI_API_KEY` (A1), `GEMINI_GEM_URL`, or any other state is shared between the template project (C12), any copy made from it, and ProPublica's canonical project (C2/C3): a copy's owner mints and stores their own key, and cannot read the template's or ProPublica's. The template itself is therefore never keyed with a working credential. Consequences for this model: C12/C13's exposure is bounded to the source code and Sheet contents ProPublica publishes there — a compromise of a copy cannot reach A1 or any other tenant, and a compromise of the template cannot read a copy's key. Each copy's own runtime posture (who its owner shares it with, what data they load, whose key funds it) belongs to that owner and is out of scope here; only the template ProPublica publishes and pushes to is in scope.
...
| Requester ↔ Template Sheet | The template Sheet (C13) is shared as **Restricted**, never "Anyone with the link" — ProPublica's Workspace disallows public sharing. Access is granted per-identity through Google's native Request-access flow, reviewed and approved individually; there is no anonymous or link-based grant, so the set of parties who can read the template (or its bound script's source) is always an explicit, auditable list. A grantee gets view access to copy from, not edit access to C13 or push access to C12 (AI-112) |
...
| C3 | Script Properties | Key-value store scoped to the script project; holds `GEMINI_API_KEY` and `GEMINI_GEM_URL` |
...
| C13 | Template Google Sheet | The public-facing Sheet users copy via `File → Make a copy`, and the container C12 is bound to — distinct from C12 itself, which is only its script. Shared as Restricted, never "Anyone with the link": access is granted individually via Google's native Request-access flow, not anonymously. Holds no credentials (see "Independence of template copies" above) and only the walkthrough content ProPublica publishes there (AI-112) |
...
| T12 | Stackdriver logging captures sensitive data | A2, A3, C2 | `appsscript.json` routes exceptions to Stackdriver. If error handling includes cell values or file names in exception messages, sensitive journalist data lands in logs accessible to anyone with GCP project access. Fixed 2026-08-17 (AI-79): `src/server/error-handling.ts`'s `logError()` is the sole logging entry point across the server (no logging existed anywhere before this). It never logs a caught exception's `.message` — only its type (`e.constructor.name`) plus caller-supplied `meta` restricted to `number \| boolean`, which cannot carry cell content, file names, or other free text |
...
| T19 | Non-secret Script Property exposed via new RPC endpoint | C1, C2, C3 | Security review (2026-08-21, AI-101): `getGeminiGemUrl()` (`src/server/index.ts`) is a new `google.script.run`-exposed function returning the raw value of a `GEMINI_GEM_URL` Script Property to any caller who can load the sidebar — the same trust boundary as every other RPC (see T8), not a new privilege level. The property is deliberately read directly rather than through `gemini-auth.ts` (the sole reader of the actual secret, `GEMINI_API_KEY`) because it isn't one: it's an administrator-configured link to a Gemini Gem, meant to be visible in the rendered UI. `PromptStep` builds the `<a href>` via DOM APIs rather than string-templating into `innerHTML`, and a follow-up commit restricts rendering to `http:`/`https:` schemes, closing the more consequential version of this gap (a misconfigured or tampered property value becoming a live `javascript:`/`data:` link). Residual risk is low and narrower than any of T1–T18: the only party who could set a malicious value already needs Script-Property write access, which is the same privilege T1/T7 already cover, and the worst outcome is a user being shown an arbitrary http(s) link, not code execution or data exfiltration |
| T20 | Attacker-directed egress via spoofed YouTube file_uri | A2, C8, S2 | AI-108 added native YouTube video support to Run AI: a `"file"`/`"auto"` prompt column cell recognized as a YouTube URL (`isValidYouTubeLink()`, `src/server/utils.ts`) is passed through **unmodified** as the Gemini request's `file_data.file_uri` — unlike a Drive link, there is no subsequent ID-extraction-and-native-API-call step to defuse a bad classification, so whatever raw string satisfies the check is exactly what gets sent to Gemini as a fetch target. Caught during implementation (TDD, before merge): the initial classifier used an unanchored substring match (mirroring `isValidDriveLink`'s intentional looseness), which a value like `https://evil.example.com/youtube.com/watch?v=xyz` would satisfy — Gemini's own infrastructure, not our server, would then be directed to fetch an attacker-chosen URL under cover of an authorized Run AI call, using the sheet as the delivery mechanism. Same shape as T11's direct-fetch vector (an in-sheet value causes a Google AI backend to fetch a URL an attacker controls) but via `file_data` rather than the `url_context` tool, and reachable by any editor who can write a cell, not gated behind an opt-in tool toggle. Fixed pre-merge (AI-108): the regex is anchored to the start of the (trimmed) cell value, requiring the host itself to be `youtube.com`/`youtu.be` rather than merely appearing as a substring anywhere in the string; `__tests__/utils.test.ts` asserts the spoofed-host case is rejected |
...
| T12 | R14 | Reduce | Audit exception handling in `index.ts` to ensure cell values, file names, and API responses are not included in thrown error messages; log structured codes rather than raw data. Implemented 2026-08-17 (AI-79) via `src/server/error-handling.ts` — see R25/R42 |
...
| T17 | R25 | Reduce | Return generic error strings from `runInference()`'s and `SSI()`'s catch blocks instead of raw exception messages; log the full exception server-side only. Implemented 2026-08-17 (AI-79): `SSI()`'s catch uses `error-handling.ts`'s `DomainError`/`toSafeMessage`/`logUnexpected`. A `DomainError` (Gemini's own structured API error, an unknown-tool validation message, the missing-API-key message) passes through verbatim; anything else becomes `[Error: ${GENERIC_FAILURE_MESSAGE}]` and is logged (type only, never the message). `GENERIC_FAILURE_MESSAGE` — "Something went wrong — try again, or contact an admin if it persists." — deliberately avoids "see script logs," since most users have sheet-edit access but not Apps Script project access. `runInference()` has no production caller (`buildInferenceRequest` + `callGeminiAPIBatch` replaced it in `runBatchAI`) and was left unchanged — tracked in AI-104 (under AI-55) for removal |
...
| T8 | R42 | Reduce | Wrap `importDriveLinks`, `extractText`, `prepRecipe`, and `sampleRowsToEvaluation` in try/catch and return scrubbed error text instead of letting raw Drive/Sheets exceptions reach the client via the default `google.script.run` failure serialization. Implemented 2026-08-17 (AI-79): each of the four, plus `runBatchAI` as a defense-in-depth backstop, wraps its body in `error-handling.ts`'s `withErrorScrubbing(site, fn)`. On any non-`DomainError` exception it logs the type server-side and throws `GENERIC_FAILURE_MESSAGE`; a `DomainError`'s message (e.g. `extractText`'s "column not found," which names a column the user selected themselves) passes through verbatim. Client panels no longer prepend their own `"Error: "` before displaying `err.message` — `google.script.run` already relays a thrown Error's `"Error: <message>"` framing, so the panels' own prefix was doubling it |
...
| T19 | R46 | Accept + document | The http(s)-scheme restriction on the client (`PromptStep`) already closes the only consequential variant of this gap; no server-side change planned. Document `GEMINI_GEM_URL` alongside `GEMINI_API_KEY` in onboarding guidance as administrator-configured and non-sensitive |
| T20 | R47 | Reduce | Anchor `isValidYouTubeLink()`'s regex to the start of the (trimmed) cell value so the host must actually be `youtube.com`/`youtu.be`, not merely present as a substring anywhere in the string. Implemented pre-merge 2026-08-27 (AI-108) alongside the feature itself — caught via a TDD test constructing a spoofed-host URL before the classifier ever shipped, so there is no window where the unanchored version was live |
...
First draft — not yet formally reviewed by the security team. A full OWASP/LLM Top 10 automated security review was conducted on 2026-07-08, surfacing threats T16–T18 and the gap items below (R21–R39). Reviewed again 2026-09-09 (AI-112) for the template Sheet distribution channel — added C12/C13, F16, the Requester ↔ Template Sheet trust boundary, the copy-independence note under Assumptions and Scope, and the T4/T7 blast-radius notes above (both threats' affected-element lists now include C12); no new threat number or open item was needed, since existing R5/R6/R9 (branch protection, limited deploy access, 2FA) already govern the second project the same way they govern the first, the template is shared Restricted rather than publicly, and each copy is a fully independent project holding no shared credential.
...
| Medium | Closed | [AI-79](https://linear.app/propublica/issue/AI-79/fix-t17-raw-exception-messages-echoed-into-spreadsheet-cells) | — | Stackdriver log audit | Done 2026-08-17 — audited while adding the first logging in the codebase; `error-handling.ts`'s `logError()` is the sole entry point and never logs a caught exception's message, only its type plus `number\|boolean`-only metadata (R14) |
...
| Low | Closed | [AI-90](https://linear.app/propublica/issue/AI-90/address-sept-2026-rejection-of-standard-keys-api-cost-control) | — | GCP monthly spend cap | Done 2026-08-17 — Gemini key minted via AI Studio (`product-ai-fellow-experiments`) with a monthly spend cap, confirmed as a hard cap; documented in the README (R16). Tied to staying on the Gemini Developer API/AI Studio — reopen if migrating to Vertex AI |
| Low | Open | — | — | Per-key daily quotas | Configure per-key daily request/token quotas in Google Cloud Console; not addressed by the monthly spend cap, which only bounds total dollar exposure over a month, not a single run's burst volume (R17) |
...
| High | Closed | [AI-78](https://linear.app/propublica/issue/AI-78/fix-t17-gemini-api-key-sent-via-url-query-string) | — | Fix T17 — API key in URL | Done 2026-07-29 — `GEMINI_API_KEY` now sent via the `x-goog-api-key` header from `src/server/gemini-auth.ts`; source-scanning guard in `__tests__/api-key-hygiene.test.ts` (R24) |
| High | Closed | [AI-79](https://linear.app/propublica/issue/AI-79/fix-t17-raw-exception-messages-echoed-into-spreadsheet-cells) | — | Fix T17 — echoed exceptions | Done 2026-08-17 — `SSI()`'s catch scrubs raw exception messages via `error-handling.ts` (R25); also closes R42 (below) and T12's Stackdriver audit (above). `runInference()` is dead code, left unchanged — tracked in AI-104 |
...
| Medium | Closed | [AI-79](https://linear.app/propublica/issue/AI-79/fix-t17-raw-exception-messages-echoed-into-spreadsheet-cells) | — | Fix T8 — missing error handling on 4 exposed functions | Done 2026-08-17 — `importDriveLinks`, `extractText`, `prepRecipe`, `sampleRowsToEvaluation` (plus `runBatchAI` as a backstop) now wrap their bodies in try/catch; raw exceptions no longer reach the client (R42) |
...
| Medium | Closed | [AI-108](https://linear.app/propublica/issue/AI-108/support-youtube-video-links-in-run-ai-fileauto-prompt-columns) | — | Fix T20 — spoofable YouTube file_uri classification | Done 2026-08-27, pre-merge — `isValidYouTubeLink()` anchored to the start of the trimmed cell value so a spoofed host (e.g. `evil.example.com/youtube.com/...`) can't pass classification and get sent to Gemini verbatim as the fetch target (R47) |
