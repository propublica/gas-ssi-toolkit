 ...
 *
 * @throws DomainError with MISSING_API_KEY_MESSAGE when the property is unset —
 *   this message is deliberately safe to show a user verbatim (see error-handling.ts)
 */
export function geminiAuthHeaders(): Record<string, string> {
  ...
  if (!apiKey) throw new DomainError(MISSING_API_KEY_MESSAGE);
